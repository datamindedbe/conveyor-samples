...
3. Populate the Oracle instance with the data you've uploaded in step 1:
   ...
   4. Wait until the data transfer is complete (adjust the placeholders USER,
      ...
      echo "SELECT text FROM table(rdsadmin.rds_file_util.read_text_file('BDUMP','dbtask-${TASKNUMBER}.log'));" \
      | docker run  \
        --rm \
        --interactive \
        oracletools/sqlplus:v19.18_lin \
   ...
   6.  Initialize this folder as a project: `conveyor project create --name dlthub`
   7.  Build the project: `conveyor build`
   8.  Deploy the project to the samples environment: `conveyor deploy --env samples --wait`
   9.  *(Add any specific commands needed to trigger or observe the sample, e.g., `conveyor run task <task_name>`, instructions to check output in S3/database)* @TODO complete
   10.  Cleanup: 
